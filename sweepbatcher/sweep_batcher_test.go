...
func testFeeBumping(t *testing.T, store testStore,
	...
	// Wait for sweep tx to appear in the wallet.
	require.Eventually(t, func() bool {
		return len(lnd.Transactions) == 1
	}, test.Timeout, eventuallyCheckFrequency)
	out1 := lnd.Transactions[0].Tx.TxOut[0].Value
	...
	// Wait for another sweep tx.
	require.Eventually(t, func() bool {
		return len(lnd.Transactions) == 2
	}, test.Timeout, eventuallyCheckFrequency)
	out2 := lnd.Transactions[1].Tx.TxOut[0].Value
